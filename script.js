...
Input.addEventListener("keyup",(event)=>{
    ...
    if (
        ...
        navigatedAndSelectProduct(event.key);
        return;
    }
    ...
    if (hasproductstoshow){
        for (let i = 0; i < filteredproduct.length; i++) {
            ...
            searchresult.append(productItemContainer);
          }
        }
...
let indexToSelect=-1;
const navigatedAndSelectProduct=(key) =>{
    ...
    if (key==="ArrowDown") {
        ...
        indexToSelect+=1;
             const productItemContainerToSelect=Select(indexToSelect);
             if (indexToSelect>0) {
             Deselect();
        };
        ...
        if (indexToSelect===-1) {
           indexToSelect= filteredproduct.length;
           const productItemContainerToSelect =  Select(indexToSelect-1);
           productItemContainerToSelect.classList.add("selected");
        };
        ...
        selectedproductcontainer.append(selectedproduct)
        container.append(selectedproductcontainer) 
    }
};
